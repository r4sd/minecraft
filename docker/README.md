...
Docker Compose で動かす Minecraft サーバー（Paper）+ プラグイン対応
...
```bash
# セットアップスクリプトをダウンロード
curl -fsSL https://raw.githubusercontent.com/YOUR_USERNAME/minecraft/main/docker/scripts/setup-host.sh -o setup-host.sh
...
# 以下を追加
0 3 * * * cd ~/minecraft/docker && ./scripts/backup.sh

# Discord通知設定（オプション）
# .envファイル作成
echo 'DISCORD_WEBHOOK=https://discord.com/api/webhooks/YOUR_WEBHOOK_URL' >> .env

# cronに環境変数を渡す
0 3 * * * cd ~/minecraft/docker && source .env && ./scripts/backup.sh
```
...
# Discord通知付き
0 3 * * * cd ~/minecraft/docker && DISCORD_WEBHOOK=https://discord.com/api/webhooks/xxx ./scripts/backup.sh
...
2. **DiscordSRV (18494)**
   - Discord連携
   - 設定ファイル: `server/plugins/DiscordSRV/config.yml`
